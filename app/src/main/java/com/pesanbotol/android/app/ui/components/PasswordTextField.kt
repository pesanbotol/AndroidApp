...
class PasswordTextField : AppCompatEditText, View.OnTouchListener {
    ...
    override fun onFocusChanged(focused: Boolean, direction: Int, previouslyFocusedRect: Rect?) {
        ...
        when {
            focused -> {
                clearButton.setTint(ContextCompat.getColor(context, R.color.blue))
            }
            !focused -> {
        ...
        addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(text: CharSequence?, p1: Int, p2: Int, p3: Int) {
                if (text?.isEmpty() == true || (text?.length ?: 0) < 6) {
                    error = context.getString(R.string.passwordLength)
                } else {
                    hideClearIcon()
                    showClearIcon()
                }
            ...
            if (isClicked) {
                when (event!!.action) {
                    MotionEvent.ACTION_UP -> {
                        ...
                        text?.clear()
                        hideClearIcon()
                        error = context.getString(R.string.password)
                        return true
                    }
                }
            }
